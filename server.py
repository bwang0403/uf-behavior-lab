...
@app.route("/participant_continue", methods=["POST"])
def participant_continue():
    global _runner
    if _runner:
...
def cmd(action):
    ...
    if action == "start":
        ...
        _runner.on_instruction = lambda text, words, cdown, title: _push("instruction", {"text": text, "words": words, "needs_countdown": cdown, "title": title})
